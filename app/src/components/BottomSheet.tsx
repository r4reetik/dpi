...
function BottomSheet({ open, setOpen, children, title, hideCloseIcon = false }: BottomSheetProp) {
  const _handleOnClose = () => setOpen(false);

  return (
    <Transition appear show={open} as={Fragment}>
      <Dialog as='div' className='fixed inset-0 z-50 overflow-hidden' onClose={_handleOnClose}>
...
const Title = ({ children }: { children: ReactNode }) => {
  return <p className='mx-2 my-3 text-xl font-extrabold tracking-wide text-white'>{children}</p>;
};

const Description = ({ children }: { children: ReactNode }) => {
  return <p className='m-2 text-sm font-medium tracking-wide text-grey-400'>{children}</p>;
};
